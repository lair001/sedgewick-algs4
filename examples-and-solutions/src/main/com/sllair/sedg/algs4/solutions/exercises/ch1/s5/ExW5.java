...
public class ExW5 {
    ...
    public int find(int p) {
        while (p != this.parent[p]) {
            ...
            this.parent[p] = this.parent[next];
            p = next;
        }
        return p;
    }
    ...
    public static void main(String[] args) {
        ...
        int n = in.readInt();
        ExW5 weightedQuickUnionPathSplitting = new ExW5(n);

        while (!in.isEmpty()) {
            int p = in.readInt();
            int q = in.readInt();

            if (!weightedQuickUnionPathSplitting.connected(p, q)) {
                weightedQuickUnionPathSplitting.union(p, q);
                StdOut.println(String.format("%d %d", p, q));
            }
        }
        StdOut.println(String.format("%d components", weightedQuickUnionPathSplitting.count()));

        if (expectedMaxRank >= 0) {
            if (weightedQuickUnionPathSplitting.maxRank() == expectedMaxRank) {
                ...
                StdOut.println(String.format("Error! Has a max rank of %d but a max rank of %d was expected!",
                        weightedQuickUnionPathSplitting.maxRank(),
                        expectedMaxRank
                ));
            }
        }
        ...
        if (expectedCount > 0) {
            if (weightedQuickUnionPathSplitting.count() == expectedCount) {
                ...
                StdOut.println(String.format("Error! Has a count of %d but a count of %d was expected!",
                        weightedQuickUnionPathSplitting.count(),
                        expectedCount
                ));
            }
        }
